package com.volextierlist.tiertagger.client.util;
        ...
        try {
            ...
            if (response.statusCode() == 200 && !response.body().isEmpty()) {
                ...
                if (jsonObj.has("id")) {
                    ...
                    VolexTierTagger.LOGGER.info("Fetched Mojang UUID for {}: {}", playerName, uuid);
                    return uuid;
                }
            }
        } catch (Exception e) {
            VolexTierTagger.LOGGER.warn("Failed to fetch Mojang UUID for {}: {}", playerName, e.getMessage());
        }
        return null;
    }
            ...
            try {
                ...
                if (response.statusCode() == 200) {
                    try (InputStream inputStream = response.body()) {
                        ...
                        VolexTierTagger.LOGGER.info("Loaded skin for {}: {}", playerName, textureId);
                        
                        return textureId;
                    }
                } else {
                    VolexTierTagger.LOGGER.warn("Failed to load skin for {}: HTTP {}", playerName, response.statusCode());
                    return null;
                }
            } catch (Exception e) {
                VolexTierTagger.LOGGER.error("Error loading skin for {}: {}", playerName, e.getMessage());
                return null;
            }
        });
    }
    ...
    public static CompletableFuture<PlayerSkinWidget> loadSkinAndCreateWidget(String playerName, MinecraftClient client) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                if (response.statusCode() != 200) {
                    VolexTierTagger.LOGGER.warn("Failed to download skin for {}: HTTP {}", playerName, response.statusCode());
                    return null;
                }
                ...
                VolexTierTagger.LOGGER.info("Created PlayerSkinWidget for {} using mineskin.eu", playerName);
                return widget;
            } catch (Exception e) {
                VolexTierTagger.LOGGER.error("Error creating skin widget for {}: {}", playerName, e.getMessage());
                return null;
            }
        });
    }
            ...
            try {
                ...
                if (response.statusCode() == 200) {
                    try (InputStream inputStream = response.body()) {
                        ...
                        VolexTierTagger.LOGGER.info("Loaded head for {}", playerName);
                        return textureId;
                    }
                } else {
                    VolexTierTagger.LOGGER.warn("Failed to load head for {}: HTTP {}", playerName, response.statusCode());
                    return null;
                }
            } catch (Exception e) {
                VolexTierTagger.LOGGER.error("Error loading head for {}: {}", playerName, e.getMessage());
                return null;
            }
        });
    }
