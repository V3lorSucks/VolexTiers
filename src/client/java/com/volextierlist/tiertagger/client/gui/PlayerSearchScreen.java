package com.volextierlist.tiertagger.client.gui;

import com.volextierlist.tiertagger.VolexTierTagger;
import com.volextierlist.tiertagger.api.TierListAPI;
import com.volextierlist.tiertagger.client.util.SkinLoader;
...
public class PlayerSearchScreen extends Screen {
    ...
    private void performSearch() {
        ...
        VolexTierTagger.LOGGER.info("[Search] Button clicked, query: '{}'", query);
        
        if (query.isEmpty()) {
            VolexTierTagger.LOGGER.info("[Search] Empty query, showing error");
            this.errorMessage = "§cPlease enter a player name";
            return;
        }

        if (this.isSearching) {
            VolexTierTagger.LOGGER.info("[Search] Already searching, ignoring");
            return;
        }

        VolexTierTagger.LOGGER.info("[Search] Starting search for: {}", query);
    ...
    private void openPlayerProfile(String playerName) {
        VolexTierTagger.LOGGER.info("[Search] openPlayerProfile called for: {}", playerName);
        if (this.client == null) {
            VolexTierTagger.LOGGER.error("[Search] Client is null!");
            return;
        }

        // Load skin and player data
        VolexTierTagger.LOGGER.info("[Search] Starting async data fetch...");
        ...
        CompletableFuture.allOf(dataFuture, skinWidgetFuture).thenRun(() -> {
            VolexTierTagger.LOGGER.info("[Search] Both futures completed");
            TierListAPI.PlayerTierData data = dataFuture.join();
            PlayerSkinWidget skinWidget = skinWidgetFuture.join();
            VolexTierTagger.LOGGER.info("[Search] Data: {}, SkinWidget: {}", data != null ? "OK" : "NULL", skinWidget != null ? "OK" : "NULL");
            ...
            if (this.client != null) {
                this.client.execute(() -> {
                    ...
                    if (data != null && skinWidget != null) {
                        VolexTierTagger.LOGGER.info("[Search] Opening PlayerInfoScreen for {}", playerName);
                        this.client.setScreen(new PlayerInfoScreen(this, data, skinWidget));
                    } else {
                        VolexTierTagger.LOGGER.warn("[Search] Player not found: {}", playerName);
                        this.errorMessage = "§cPlayer not found: " + playerName;
                    }
                });
            }
        }).exceptionally(throwable -> {
            VolexTierTagger.LOGGER.error("[Search] Exception during search: {}", throwable.getMessage(), throwable);
            if (this.client != null) {
                this.client.execute(() -> {
                    this.isSearching = false;
                    this.searchButton.active = true;
